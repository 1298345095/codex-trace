...
### Codex CLI subcommand boundary

codex-trace **never invokes any Codex CLI subcommand**. It reads session data exclusively from local JSONL files written by the Codex CLI to `~/.codex/sessions/`. It does not shell out to `codex`, pipe its output, or call any Codex CLI subcommand at runtime.

This means Codex CLI subcommand removals do not affect codex-trace. Specifically, the removal of the `codex responses` subcommand in Codex v0.128.0 (PR #19640) does **not** affect codex-trace.

**Important naming clarification**: the `response_item` entry type that appears throughout the parser (`src-tauri/src/parser/`) is a JSONL log line type written by the Codex CLI into session files — it is unrelated to the removed `codex responses` CLI subcommand. The parser reads these log lines from disk; it never calls any CLI command to produce them.
